...
const char* HexNode::statestring[] = { ".", "B", "W" };
...
const char* HexGraph::WINNERSTATE[] = { "DRAW", "COMPUTER", "PLAYER" };
...
HexGraph::pickPosition(const std::tuple<int, int>& pos, HexNode::state targetstate)
{
    ...
    --remaining_;
    if (targetstate == HexNode::WHITE) {
	mergegroup(node, playergroup_);
    } else if (targetstate == HexNode::BLACK) {
...
HexGraph::findmostconnected(HexNode::state targetstate, int threshold) const
{
    ...
    //to the target state
    auto& group = (targetstate == HexNode::WHITE)?playergroup_:computergroup_;
	...
	for (auto it = nnlist.begin(); it != nnlist.end(); ++it)
	{
		...
		if (targetstate == HexNode::WHITE && isConnected(left_, (*it), HexNode::WHITE) && 
			isConnected((*it), right_, HexNode::WHITE))
		{
...
{
    bool res = false;
    if (isConnected(top_, bottom_, HexNode::BLACK))
    {
	winner_ = HexNode::BLACK;
	res = true;
    } else if (isConnected(left_, right_, HexNode::WHITE)) {
	winner_ = HexNode::WHITE;
	res = true;
...
HexGraph::HexGraph(int size) : size_(size), remaining_(size*size), winner_(HexNode::OPEN)
{
    assert(size > 0);
...
int main()
{
    ...
    std::cout << "Player (white) moves horizontally, and computer goes vertically " << std::endl;
    HexGraph hgraph(size);
    if (user.getFirst())
    {
	hgraph.makeMove();
    }
    ...
    {
	hgraph.draw();
	hgraph.pickPosition(user.getNextMove(hgraph), HexNode::WHITE);
	if (!hgraph.endGame())
	   hgraph.makeMove(); 
    }
    std::cout << "======== end of Game ============" << std::endl;
    std::cout << "Winner is : " << hgraph.getWinner() << std::endl;
    hgraph.draw();
    return 0;
}
